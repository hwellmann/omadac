 ...
 * </li>
 * <li>
 *      System will call {@link #map(List, Object) map(List, Object)}. This
 ...
 *          Note that if you use {@link GridTaskAdapter} adapter for <tt>GridTask</tt>
 ...
 * <tt>GridTask</tt> comes with several convenience adapters to make the usage easier:
 * <ul>
 * <li>
 ...
 * <p>
 * <h1 class="header">Map/Reduce</h1>
 * The design of <tt>GridTask</tt> is influenced by Google <tt>Map/Reduce</tt> paradigm.
 * For more information about <tt>Map/Reduce</tt> paradigm, refer to
 * <a href="http://labs.google.com/papers/mapreduce.html">Map/Reduce: Simplified Data Processing on Large Clusters</a>
 * article from Google. Note, however, that in Google terminology task means GridGain's job and job
 * means GridGain's task. Following diagram shows basic idea behind Map/Reduce:
 * <p>
 * <img src="http://www.gridgain.com/images/mapreduce_white.png">
 * <p>
 * <ol>
 *      <li>Task execution request
 *      <li>Task splits (maps) into jobs
 *      <li>Result of job execution
 *      <li>Aggregation (reduction) of job results into task result
 * </ol>
 * <h1 class="header">Examples</h1>
 * Many task example usages are available on GridGain <a href="http://wiki.gridgain.org" target="_top">Wiki</a>.
 * To see example on how to use <tt>GridTask</tt> for basic split/aggregate logic refer to
 ...
 * For example on how to use <tt>GridTask</tt> with automatic grid-enabling via
 * {@link org.gridgain.grid.gridify.Gridify @Gridify} annotation refer to
...
public interface GridTask<T, R> extends Serializable {
     ...
     *
     * @param arg Task execution argument. Can be <tt>null</tt>. This is the same argument
     *      as the one passed into {@link Grid#execute(GridTask, Object)} method (or any of
     *      the other <tt>Grid.execute(...)</tt> methods.
     ...
     * @return Map of grid jobs assigned to subgrid node. If <tt>null</tt> or empty list
     ...
     */
    public Map<? extends GridJob, GridNode> map(List<GridNode> subgrid, @Nullable T arg) throws GridException;
     ...
     * @param received All previously received results. Note that if task class has
     *      {@link GridTaskNoResultCache} annotation, actual job result won't be cached
     *      and {@link GridJobResult#getData()} method will return <tt>null</tt>.
     ...
     *      and {@link GridJobResult#getData()} methods will return <tt>null</tt>.
     ...
     *      be thrown out of {@link GridTaskFuture#get()} method.
     */
    public R reduce(List<GridJobResult> results) throws GridException;
}
