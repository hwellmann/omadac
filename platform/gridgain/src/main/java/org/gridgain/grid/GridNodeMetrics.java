 ...
 * metrics on local node {@link Grid#getLocalNode() Grid.getLocalNode().getMetrics()}
...
public interface GridNodeMetrics extends Serializable {
     ...
     *      operations.
     */
    @GridMBeanDescription("Total number of jobs this node rejects during collision resolution operations.")
     ...
     * it's ultimately upto the job itself to gracefully exit.
     *
     ...
     */
    @GridMBeanDescription("The number of processors available to the virtual machine; never smaller than one.")
     ...
     * </ul>
     *  
     * @return Average of CPU load values over all metrics kept in the history.
     */
     ...
     * @return The maximum amount of memory in bytes; <tt>-1</tt> if undefined.
     */
    @GridMBeanDescription("Maximum amount of non-heap memory in bytes that can be used for memory management. -1 if undefined.")
